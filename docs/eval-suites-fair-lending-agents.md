...
**Status: designed, not yet executed against the built tool.**
...
| A-06 | Very short, boilerplate-thin notice with only "application denied" and nothing else | Flags 3 of 4 missing elements | Same |

**Status:** designed, zero cases run. Running this requires (a) an actual
...
the Model Risk Register until both of those happen.** Fallback mode's
cases (A-01, A-02, A-03, A-05, A-06) can be run and graded right now with
no API key; A-04 is the one case that specifically tests whether live mode
earns its keep over fallback, and it's the one that most needs a second
human opinion on what "correct" even means before grading anything against
it.
